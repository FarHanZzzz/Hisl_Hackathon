...
class GaitScanner:
    def __init__(self, model_path='pose_landmarker_lite.task'):
        ...
        Default model: pose_landmarker_lite.task (optimized for i3 7th gen CPU).
        """
    ...
    def process_frame(self, image, show_skeleton=True, angle_diff_alert=30):
        """
        Processes a single frame using Mediapipe Tasks API.

        Args:
            image: BGR image from OpenCV.
            show_skeleton: Whether to draw skeleton overlay on the image.
            angle_diff_alert: Degree threshold to trigger asymmetry alert text.
        ...
        try:
            ...
            # Draw skeleton only if requested
            if show_skeleton:
                h, w, c = image.shape
                ...
                if left_angle is not None:
                    cv2.putText(image, f'{left_angle:.0f}', to_pix(left_knee),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
                if right_angle is not None:
                    cv2.putText(image, f'{right_angle:.0f}', to_pix(right_knee),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

            # Asymmetry Check (always drawn if detected, regardless of skeleton toggle)
            if left_angle is not None and right_angle is not None:
                if abs(left_angle - right_angle) > angle_diff_alert:
